...
function EmployeeOverallRatingTimelineChart() {
  const { cWrapper } = useCatch();
  const [isOpen, setIsOpen] = useState(true);
  const { filterValues, triggerOverallRatingFetch } = useEmployeeFilter();
  ...
  // Fetch data when triggerOverallRatingFetch changes or employee/date range changes
  useEffect(() => {
  ...
  }, [triggerOverallRatingFetch, cWrapper]);
    ...
    const chartDataPoints = chartResponseData.overallRatings
      ...
      .map(period => {
        ...
        return {
          ...
          overallRating: period.overallRating || 0
        };
      })
    ...
    <Card className="mb-4">
      ...
      >
        <div className="d-flex justify-content-between align-items-center">
          <h4 className="mb-0">Employee Overall Rating Timeline</h4>
          ...
          <>
            {chartResponseData && chartData.length === 0 && (
              <div className="mt-4 text-muted">
                <p>No overall rating data available for the selected employee{filterValues.startDate && filterValues.endDate ? ` and date range` : ''}.</p>
              </div>
            )}
            {!chartResponseData && (
              <div className="mt-4 text-muted">
                <p>No overall rating data found for the selected employee{filterValues.startDate && filterValues.endDate ? ` and date range` : ''}. Please try a different employee or date range.</p>
              </div>
            )}
          </>
        )}
...
export default EmployeeOverallRatingTimelineChart;
